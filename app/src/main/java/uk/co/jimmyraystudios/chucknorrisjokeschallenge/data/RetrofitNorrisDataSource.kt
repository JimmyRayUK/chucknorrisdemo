...
class RetrofitNorrisDataSource(val service : RetrofitNorrisService) : NorrisDataSource {
    ...
    override fun getCustomNameJoke(listener: NorrisDataSource.ResponseListener, lastName : String, firstName : String) {
        val argMap = HashMap<String, String>()
        if(firstName.isNotEmpty()) {
            argMap.put(RetrofitNorrisService.QUERY_FIRST_NAME, firstName)
        }
        if(lastName.isNotEmpty()) {
            argMap.put(RetrofitNorrisService.QUERY_LAST_NAME, lastName)
        }
